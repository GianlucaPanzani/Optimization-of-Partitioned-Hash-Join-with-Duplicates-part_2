...
\begin{itemize}
    \item \texttt{hashjoin\_seq.cpp}: sequential reference (baseline);
...
\begin{itemize}
    ...
    \item \texttt{partition\_threads} and \texttt{join\_threads} vary over \(\{4,8,16,32,64\}\) for the parallelized versions, while the sequential baseline with one thread.
\end{itemize}
...
Scaling is strong up to 32 threads for the partition phase and up to 16 threads for the join phase, then gains are smaller. This is coherent with the partition plateau and with increased overhead/imbalance sensitivity in the join stage at high thread counts.
...
Each \((N,T)\) point is repeated 5 times, then averaged to avoid unstabile measures.
...
\end{itemize}
This indicates good scalability trend with non-ideal efficiency at high thread counts, consistent with memory traffic and synchronization overheads.
