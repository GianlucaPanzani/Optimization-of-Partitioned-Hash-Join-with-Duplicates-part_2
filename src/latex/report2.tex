...
\begin{itemize}
    ...
    \item \texttt{hashjoin\_par\_p.cpp}: parallel partition, sequential join;
    \item \texttt{hashjoin\_par\_pj.cpp}: parallel partition + parallel join (contiguous partition ranges);
    \item \texttt{hashjoin\_par\_pj\_wb.cpp}: adds workload-balanced assignment of partitions;
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\linewidth]{../img/007_partition_throughput_matrix_nr50000000_ns50000000_maxkey1000000.png}
    \caption{Partition-throughput matrix under fixed problem size.}
    \label{fig:strong-partition}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.70\linewidth]{../img/008_join_throughput_matrix_nr50000000_ns50000000_maxkey1000000.png}
    \caption{Join-throughput matrix under fixed problem size.}
    \label{fig:strong-join}
\end{figure}
...
\begin{wrapfigure}{r}{0.38\textwidth}
    \vspace{-1.0em}
    \centering
    \includegraphics[width=0.30\textwidth]{../img/004_std_join_time_by_max_key.png}
...
Strict weak scaling requires increasing input size proportionally to the number of threads while keeping work per thread constant.  
The current campaign uses a fixed size (\(nr=ns=50\)M), so it is not a full weak-scaling experiment.

To keep this section grounded on available data, the global throughput matrix can be read as a scaling \textit{proxy} while resources increase:
\begin{figure}[H]
    \centering
    \includegraphics[width=0.62\linewidth]{../img/009_total_throughput_matrix_nr50000000_ns50000000_maxkey1000000.png}
    \caption{Total throughput matrix used as weak-scaling proxy under fixed \(N\).}
    \label{fig:weak-proxy}
\end{figure}

The figure shows higher throughput as thread counts increase up to the 32--64 range, with reduced marginal gains at the top end. A true weak-scaling study still requires new runs where \(N\) grows with thread count (\(N \propto T\)).
